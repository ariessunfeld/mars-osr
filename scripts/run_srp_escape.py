"""Run SRP-driven Mars escape scenarios.

The default case uses a sigma=18 g/m² sail, a polar 1500 km initial orbit,
reactive dE/dt-max steering, kinematic slew limits of 0.003 deg/s² and
0.3 deg/s, Mars J2, and the Sun as a third body. The 20 s absolute step ceiling
and 0.25 deg true-anomaly cap resolve the attitude dynamics as the orbit
expands; ``--max_step_s 40`` provides a faster exploratory setting.

Escape is defined by both nonnegative planet-relative energy and Hill-radius
crossing. ``--no-energy_gated`` selects radius-only termination for comparison.
Alternate sails, planes, altitudes, and controllers are supported but are not
guaranteed to escape.

Usage:
    python scripts/run_srp_escape.py                      # reproduce the escape
    python scripts/run_srp_escape.py 10 --tag sig10       # lighter sail
    python scripts/run_srp_escape.py 18 --max_step_s 40 --tag quick   # ~2x faster
    python scripts/run_srp_escape.py 18 --controller blended --plane ecliptic

Controllers: naive (default; reactive no-guard dE/dt-max), blended (M-M energy
+ periapsis safety), and dedot (dE/dt-max + conditional rp_warn guard).
Outputs: per-step CSV (full orbit + attitude history) + summary (+ Hill handoff
state on escape) under simulation_outputs/ (gitignored).
"""
...
from reflectors.elements import classical_elements, state_from_classical_mme2000
...
from reflectors.sun_sync import raan_mme2000_from_ltan, sun_sync_inclination_rad
...
def sun_sync_ltan_state(epoch_et, altitude_km, ltan_h, mu):
    """Circular SUN-SYNCHRONOUS orbit at a given LTAN (Mars-equator-referenced).

    The Mars analogue of a dawn-dusk Earth escape:
    inclination = Brouwer first-order sun-sync inclination for this altitude
    (``sun_sync_inclination_rad``, retrograde, w.r.t. the Mars equator), node
    placed at the requested Local Time of the Ascending Node
    (``raan_mme2000_from_ltan``); LTAN 18 = dusk (Sun ~perpendicular to the
    plane). e=0, nu=0 (start at the ascending node) at ``epoch_et``. Built via
    ``state_from_classical_mme2000`` (elements in MME2000 -> state returned in
    Mars-centred J2000, the frame ``propagate_escape`` uses). Returns
    ``(state6, i_rad)`` so the caller can print the (MME2000) sun-sync i -- the
    J2000-osculating i printed elsewhere differs (~59 deg) only because Mars's
    pole is tilted ~37 deg from J2000; the orbit IS sun-sync in the Mars frame.
    """
    a_km = mars_equatorial_radius_km() + altitude_km
    i_rad = sun_sync_inclination_rad(a_km)
    raan = raan_mme2000_from_ltan(ltan_h, epoch_et)
    state6 = state_from_classical_mme2000(
        a_km=a_km, e=0.0, inclination_rad=i_rad, raan_rad=raan,
        argp_rad=0.0, nu_rad=0.0, mu_km3_s2=mu, epoch_et=epoch_et)
    return state6, i_rad
...
ap.add_argument("--ltan", type=float, default=None,
                help="If set, start from a SUN-SYNCHRONOUS orbit at this Local "
                     "Time of Ascending Node (hours): i = sun-sync inclination for "
                     "the altitude (Mars-equator-referenced), node at the LTAN. "
                     "18 = dawn-dusk (Sun ~perp to plane); the reflector-orbit "
                     "analogue of the dawn-dusk Earth escape. Overrides --plane. "
                     "Default None = the --plane construction (unchanged).")
...
ap.add_argument("--epoch", type=str, default=EPOCH,
                help="UTC start epoch. Default 2028-01-01 (the reference "
                     "perihelion-ish epoch). Sweep this for the orbital-phase "
                     "(Mars-year) escape-time / exit-direction characterization.")
...
ap.add_argument("--ephemeris_time_direction", type=int, default=1, choices=[1, -1],
                help="+1 (default) = forward escape; -1 = clock the Sun/3-body "
                     "ephemeris BACKWARD -> the spiral-out is the time-reverse of "
                     "a forward-Sun CAPTURE (handoff velocity is -v). Mars mission "
                     "leg is a capture -> use -1 for the Mars-Hill capture node.")
...
ltan_tag = f"_ssoLTAN{args.ltan:g}".replace(".", "p") if args.ltan is not None else ""
tag = f"sigma{int(round(sigma_g))}_{args.plane}_{args.controller}{ltan_tag}{suffix}"
...
if args.ltan is not None:
    state0, _sso_i_rad = sun_sync_ltan_state(
        epoch_et, args.altitude_km, args.ltan, mu_mars)
    print(f"[init] sun-sync LTAN={args.ltan:g}h: i_sso(Mars equator)="
          f"{math.degrees(_sso_i_rad):.3f} deg  (J2000-osculating i printed below "
          f"differs by Mars's ~37deg pole tilt -- orbit IS sun-sync)", flush=True)
elif args.plane == "polar":
    state0 = polar_state(epoch_et, args.altitude_km, mu_mars)
else:
    state0 = initial_circular_state(args.altitude_km, epoch_et)

if args.ephemeris_time_direction == -1:
    # BACKWARD capture: the flown forward capture is the time-reverse of this
    # spiral-out run, so it ARRIVES with -v. To land in the DESIRED orbit
    # (r, +v) built above (e.g. the retrograde sun-sync LMO, not its prograde
    # supplement, which is not sun-sync), the backward run must START at (r, -v).
    # Without this the capture delivers the time-reversed supplement orbit.
    # The start elements printed below are therefore
    # the supplement; the forward-capture ARRIVAL is the desired orbit.
    state0 = time_reversed_capture_initial_state(state0)
    print("[init] BACKWARD capture: negated initial velocity so the forward "
          "capture arrives in the desired orbit (printed start elements are the "
          "time-reversed supplement)", flush=True)
...
with open(csv_path, "w", newline="") as fh:
    ...
    for i in range(0, n_steps, stride):
        t = float(res.t_s[i])
        # Ephemeris epoch: epoch + dir*t (dir=-1 = backward-clocked capture run).
        # Forward runs (dir=+1, the default) are byte-identical. Without the dir
        # factor the CSV et column -- and every Sun/shadow/SRP diagnostic derived
        # from it -- would have the opposite sign for backward captures.
...
lines = [
    f"SRP Mars-escape -- sigma={sigma_g:.1f} g/m^2  plane={args.plane}  "
    ...
    f"ephemeris direction: {args.ephemeris_time_direction:+d} "
    f"({'forward escape' if args.ephemeris_time_direction > 0 else 'BACKWARD capture'})",
...
if res.termination_t_s is not None:
    ...
    lines += [f"event time         : {t_esc/SECONDS_PER_SOLAR_SOL_S:.1f} sols "
...
if res.escaped:
    ...
    lines += [
        "",
        "=== Hill-sphere handoff state (J2000, Mars-centred) ===",
